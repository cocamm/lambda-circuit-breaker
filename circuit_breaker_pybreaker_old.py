...
class CircuitRedisStorage(CircuitBreakerStorage):
    ...
    def get_counter_timer(self, window_size):
        """
        Returns the current value of the failure counter.
        """
        try:
            ...
            p.zremrangebyscore(key, "-inf", int(t) - window_size)
            p.zcard(key)
            result = p.execute()
            value = result.pop()
            if value:
                logging.info(f'counter_value={value}')
                return int(value)
            else:
                return 0
        except self.RedisError:
    ...
    def get_fail_counter_timer(self, window_size):
        """
        Returns the current value of the failure counter.
        """
        try:
            ...
            p = self._redis.pipeline()
            p.zremrangebyscore(key, "-inf", int(timer) - window_size)
            p.zcard(key)
            result = p.execute()
            value = result.pop()
            if value:
                print(f'fail_counter_value={value}')
                return int(value)
            else:
                return 0
        except self.RedisError:
...
class CircuitBreakerTimerAggregator:
    ...
    def record(self, result: ResultType):
        ...
        self.__total_count = self.__breaker.get_counter_timer(timer,
            self.__window_size)
        self.__fail_count = self.__breaker.get_fail_counter_timer(timer,
            self.__window_size)
